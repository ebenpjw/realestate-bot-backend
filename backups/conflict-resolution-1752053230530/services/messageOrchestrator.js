...
class MessageOrchestrator {
  ...
  /**
   * Main entry point - replaces direct botService.processMessage() calls
  ...
  async _processBatchedMessages(senderWaId, messages) {
    ...
    try {
      ...
      logger.error({
        ...
        senderWaId,
        batchSize: messages.length
      }, '[ORCHESTRATOR] Error in unified processing - falling back to individual processing');
  ...
  async _emergencyFallback(senderWaId, messageData = null) {
    ...
    }, '[ORCHESTRATOR] Emergency fallback to direct processing');
    ...
    try {
      if (messageData) {
        ...
        // Process latest message from queue if available
        const queue = this.messageQueues.get(senderWaId);
        if (queue && queue.messages.length > 0) {
          const latestMessage = queue.messages[queue.messages.length - 1];
          await botService.processMessage({
            senderWaId,
            userText: latestMessage.userText,
            senderName: latestMessage.senderName
      ...
      logger.error({
        err: error,
        senderWaId
      }, '[ORCHESTRATOR] Error in emergency fallback');
    } finally {
      this._cleanupProcessing(senderWaId);
    }
  }
