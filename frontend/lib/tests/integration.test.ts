...
const mockApiResponses = {
  ...
  testing: {
    activeTests: [
      {
        ...
        scenario: 'Difficult Singaporean Investor',
        status: 'active',
        messages: [
          {
            id: 'msg_1',
            sender: 'lead',
            message: 'Hello, I am looking for investment properties',
    ...
    insights: {
      responseAccuracy: 85.2,
      conversionEffectiveness: 76.8,
      engagementQuality: 88.1,
      commonIssues: [
        {
          issue: 'Slow response to price objections',
          frequency: 12,
          impact: 'medium',
          suggestion: 'Prepare more price justification templates'
        }
      ]
    }
  }
...
describe('Frontend-Backend Integration Tests', () => {
  ...
  describe('Testing Interface Integration', () => {
    it('should load active tests without sending actual WhatsApp messages', async () => {
      mockApiClient.get.mockResolvedValue({
        data: { data: mockApiResponses.testing }
      })

      const { result } = renderHook(() => useTestingOverview('agent_1'), {
        wrapper: TestWrapper
      })
      ...
      expect(result.current.activeTests[0].scenario).toBe('Difficult Singaporean Investor')
      expect(result.current.analytics?.totalTests).toBe(45)
    })

    it('should handle test message sending in simulation mode', async () => {
      mockApiClient.post.mockResolvedValue({
        data: {
          data: {
            ...
            botResponse: {
              message: 'Thank you for your interest! I can help you find the perfect investment property.',
              responseTime: 1.2,
              confidence: 0.95
            }
          }
      ...
      // Verify API structure for test message sending
