-- ============================================================================
-- FACEBOOK/INSTAGRAM LEAD INTEGRATION SCHEMA MIGRATION
...
-- 
-- CHANGES:
-- 1. Enhanced leads table with better attribution and source tracking
-- 2. New facebook_pages table for agent page connections
-- 3. New lead_sources table for detailed source attribution
-- 4. New lead_deduplication table for duplicate management
...
-- Add new columns to leads table for better attribution
DO $$
BEGIN
    -- Add columns only if they don't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'leads' AND column_name = 'primary_source') THEN
        ALTER TABLE leads ADD COLUMN primary_source VARCHAR(100) DEFAULT 'whatsapp_direct';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'leads' AND column_name = 'source_details') THEN
        ALTER TABLE leads ADD COLUMN source_details JSONB DEFAULT '{}'::jsonb;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'leads' AND column_name = 'lead_quality_score') THEN
        ALTER TABLE leads ADD COLUMN lead_quality_score INTEGER DEFAULT 0;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'leads' AND column_name = 'first_contact_method') THEN
        ALTER TABLE leads ADD COLUMN first_contact_method VARCHAR(50);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'leads' AND column_name = 'lead_temperature') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'leads' AND column_name = 'is_merged') THEN
        ALTER TABLE leads ADD COLUMN is_merged BOOLEAN DEFAULT false;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'leads' AND column_name = 'merged_from_lead_ids') THEN
        ALTER TABLE leads ADD COLUMN merged_from_lead_ids JSONB DEFAULT '[]'::jsonb;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'leads' AND column_name = 'duplicate_check_hash') THEN
        ALTER TABLE leads ADD COLUMN duplicate_check_hash VARCHAR(64);
    END IF;
END $$;

-- Add constraints for new columns (using DO block for conditional constraint creation)
DO $$
BEGIN
    -- Add lead_temperature constraint if it doesn't exist
    IF NOT EXISTS (
...
END
WHERE primary_source = 'whatsapp_direct' AND source IS NOT NULL;
