-- ============================================================================
-- SAFE PARTNER API MIGRATION FOR PRODUCTION
-- This migration handles existing data gracefully and prevents startup errors
-- ============================================================================
...
-- ============================================================================
-- 1. ENSURE AGENTS TABLE HAS REQUIRED STATUS COLUMN FIRST
-- This prevents startup errors from services querying agents.status
-- ============================================================================

-- Add status column to agents if it doesn't exist (this is critical for startup)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'agents' AND column_name = 'status') THEN
        ...
        -- Update any existing agents to have active status
        UPDATE agents SET status = 'active' WHERE status IS NULL;
    END IF;
END $$;

-- Add check constraint for agents status
DO $$
BEGIN
    ALTER TABLE agents DROP CONSTRAINT IF EXISTS agents_status_check;
    ALTER TABLE agents ADD CONSTRAINT agents_status_check CHECK (status IN ('active', 'inactive', 'suspended'));
END $$;

-- ============================================================================
-- 2. ADD OTHER WABA COLUMNS TO AGENTS TABLE
-- ============================================================================
...
-- Add check constraint for waba_status
DO $$
BEGIN
...
-- ============================================================================
-- 3. CREATE ORGANIZATIONS TABLE FIRST (needed for foreign keys)
-- ============================================================================
...
-- ============================================================================
-- 5. CREATE TEMPLATE USAGE LOG TABLE
-- ============================================================================
...
-- ============================================================================
-- 6. CREATE PARTNER API TOKENS TABLE
-- ============================================================================
...
-- Critical indexes for startup queries
...
-- Set default status for existing agents
UPDATE agents 
SET status = 'active'
...
SELECT 'Safe Partner API migration completed successfully!' as result;
